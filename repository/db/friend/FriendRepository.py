...
class FriendRepository(MultiDbRepository):
	...
	def getCount(self, scode, userId, friendType):
		if('all' == friendType):
			return super().count(f"SELECT COUNT(*) FROM friend WHERE userId='{userId}'")
		return super().count(f"SELECT COUNT(*) FROM friend WHERE userId='{userId}' AND friendType='{friendType}'")

	def getFriendMeCount(self, scode, userId, friendType):
		if('all' == friendType):
			return super().count(f"SELECT COUNT(*) FROM friend WHERE friendId='{userId}'")
		return super().count(f"SELECT COUNT(*) FROM friend WHERE friendId='{userId}' AND friendType='{friendType}'")
